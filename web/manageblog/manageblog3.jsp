...
<div>
    <div id="zuo">
        <ul class="nav nav-pills nav-stacked">
            ...
            <hr>
            <li role="presentation" class="active"><a href="/manageblogServlet3">删除博客</a></li>
        </ul>
    </div>
    <div id="you" >
        <br>
        ...
        <c:if test="${page.totalpage!=0}">
            <c:forEach items="${page.list}" var="user" varStatus="s">
                <div>
                    ...
                    <button type="button" id="remove${user.id}" onclick="remove(${user.id})" class="btn btn-default navbar-btn" style="border-color: brown;margin-left: 420px;margin-top:-30px">删除博客</button>
                </div>
                <hr>
            </c:forEach>
            <nav aria-label="Page navigation" style="text-align: center">
                <ul class="pagination">
                    ...
                    <li>
                        ...
                        <a href="${pageContext.request.contextPath}/manageblogServlet3?nowpage=${page.nowpage-1}&count=5" aria-label="Previous">
                    ...
                    <c:if test="${page.nowpage-2<=1}">
                        <c:forEach begin="1" end="${page.nowpage}" var="i">
                            <c:if test="${page.nowpage==i}">
                                <li class="active"><a href="${pageContext.request.contextPath}/manageblogServlet3?nowpage=${i}&count=5">${i}</a></li>
                            </c:if>
                            <c:if test="${page.nowpage!=i}">
                                <li><a href="${pageContext.request.contextPath}/manageblogServlet3?nowpage=${i}&count=5">${i}</a></li>
                            </c:if>
                        </c:forEach>
                    </c:if>
                    <!--前半部分有多余页-->
                    <c:if test="${page.nowpage-2>1}">
                        <c:if test="${1==page.nowpage}">
                            <li class="active"><a href="${pageContext.request.contextPath}/manageblogServlet3?nowpage=1&count=5">1</a></li>
                        </c:if>
                        <c:if test="${1!=page.nowpage}">
                            <li><a href="${pageContext.request.contextPath}/manageblogServlet3?nowpage=1&count=5">1</a></li>
                        </c:if>
                        <li><a>...</a></li>
                    ...
                    <c:if test="${page.nowpage+2<page.totalpage}">
                        ...
                        <c:if test="${page.totalpage==page.nowpage}">
                            <li class="active"><a href="${pageContext.request.contextPath}/manageblogServlet3?nowpage=${page.totalpage}&count=5">${page.totalpage}</a></li>
                        </c:if>
                        <c:if test="${page.totalpage!=page.nowpage}">
                            <li><a href="${pageContext.request.contextPath}/manageblogServlet3?nowpage=${page.totalpage}&count=5">${page.totalpage}</a></li>
                        </c:if>
                    </c:if>
                    <c:if test="${page.nowpage+2>=page.totalpage}">
                    ...
                    <li>
                        ...
                        <a href="${pageContext.request.contextPath}/manageblogServlet3?nowpage=${page.nowpage + 1}&count=5" aria-label="Next">
