...
<head>
    ...
    <script>
        ...
        function recommend(blogid){
            if(document.getElementById("recommend"+blogid).innerHTML=="取消推荐"){
                document.getElementById("recommend"+blogid).innerHTML="推荐博客";
                $.post("/recommendServlet",{flag:0,blogid:blogid},function () {
                    $("#total").load("/manageblogServlet2");
                })
            }else{
                document.getElementById("recommend"+blogid).innerHTML="取消推荐";
                $.post("/recommendServlet",{flag:1,blogid:blogid},function () {
                    $("#total").load("/manageblogServlet2");
                })
            }
        }
...
<div>
    <div id="zuo">
        <ul class="nav nav-pills nav-stacked">
            <li role="presentation"><a href="/manageblogServlet">审核博客</a></li>
            <hr>
            <li role="presentation" class="active"><a href="/manageblogServlet2">推荐博客</a></li>
            <hr>
            <li role="presentation"><a href="/manageblogServlet3">删除博客</a></li>
        </ul>
    </div>
        ...
        <c:if test="${page.totalpage!=0}">
            <c:forEach items="${page.list}" var="user" varStatus="s">
                <div>
                    ...
                    <c:if test="${user.recommend==0}">
                        <button type="button" id="recommend${user.id}" onclick="recommend(${user.id})" class="btn btn-default navbar-btn" style="border-color: brown;margin-left: 420px;margin-top:-30px">推荐博客</button>
                    </c:if>
                    <c:if test="${user.recommend==1}">
                        <button type="button" id="recommend${user.id}" onclick="recommend(${user.id})" class="btn btn-default navbar-btn" style="border-color: brown;margin-left: 420px;margin-top:-30px">取消推荐</button>
                    </c:if>
            ...
            <nav aria-label="Page navigation" style="text-align: center">
                <ul class="pagination">
                    ...
                    <li>
                        ...
                        <a href="${pageContext.request.contextPath}/manageblogServlet2?nowpage=${page.nowpage-1}&count=5" aria-label="Previous">
                    ...
                    <c:if test="${page.nowpage-2<=1}">
                        <c:forEach begin="1" end="${page.nowpage}" var="i">
                            <c:if test="${page.nowpage==i}">
                                <li class="active"><a href="${pageContext.request.contextPath}/manageblogServlet2?nowpage=${i}&count=5">${i}</a></li>
                            </c:if>
                            <c:if test="${page.nowpage!=i}">
                                <li><a href="${pageContext.request.contextPath}/manageblogServlet2?nowpage=${i}&count=5">${i}</a></li>
                            </c:if>
                        </c:forEach>
                    </c:if>
                    <!--前半部分有多余页-->
                    <c:if test="${page.nowpage-2>1}">
                        <c:if test="${1==page.nowpage}">
                            <li class="active"><a href="${pageContext.request.contextPath}/manageblogServlet2?nowpage=1&count=5">1</a></li>
                        </c:if>
                        <c:if test="${1!=page.nowpage}">
                            <li><a href="${pageContext.request.contextPath}/manageblogServlet2?nowpage=1&count=5">1</a></li>
                        </c:if>
                        <li><a>...</a></li>
                    ...
                    <c:if test="${page.nowpage+2<page.totalpage}">
                        ...
                        <c:if test="${page.totalpage==page.nowpage}">
                            <li class="active"><a href="${pageContext.request.contextPath}/manageblogServlet2?nowpage=${page.totalpage}&count=5">${page.totalpage}</a></li>
                        </c:if>
                        <c:if test="${page.totalpage!=page.nowpage}">
                            <li><a href="${pageContext.request.contextPath}/manageblogServlet2?nowpage=${page.totalpage}&count=5">${page.totalpage}</a></li>
                        </c:if>
                    </c:if>
                    <c:if test="${page.nowpage+2>=page.totalpage}">
                    ...
                    <li>
                        ...
                        <a href="${pageContext.request.contextPath}/manageblogServlet2?nowpage=${page.nowpage + 1}&count=5" aria-label="Next">
