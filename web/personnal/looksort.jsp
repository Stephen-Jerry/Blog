    ...
    <script>
        ...
        function remove(blogid){
            $.post("/removeblogServlet",{blogid:blogid},function () {
                $("#total").load("/looksortServlet?sortname=${sessionScope.sortname}&userid=${sessionScope.userid}");
            })
        }
    </script>
</head>
    ...
    <div class="container-fluid you" style="min-height: 100px" id="bloglist">
        ...
        <c:if test="${page.totalpage!=0}">
            <c:forEach items="${page.list}" var="user" varStatus="s">
                <div>
                    <a href="/lookblogServlet?blogid=${user.id}&headline=${user.head}&types=${user.types}&look=${user.look}&publicc=${user.publicc}&bloguserid=${user.userid}&userid=${userid}&hide=${user.hide}&good=${user.good}&state=${user.state}" target="_blank"><h3 style="margin-top: 0px">${user.head}</h3></a>
                    ...
                    <a href="/modifyblogServlet?blogid=${user.id}&headline=${user.head}&types=${user.types}&userid=${id}" style="margin-left: 500px">[编辑]</a>
            ...
            <nav aria-label="Page navigation" style="text-align: center">
                <ul class="pagination">
                    ...
                    <li>
                        ...
                        <a href="${pageContext.request.contextPath}/looksortServlet?nowpage=${page.nowpage-1}&count=5&sortname=${sessionScope.sortname}&userid=${sessionScope.userid}" aria-label="Previous">
                    ...
                    <c:if test="${page.nowpage-2<=1}">
                        <c:forEach begin="1" end="${page.nowpage}" var="i">
                            <c:if test="${page.nowpage==i}">
                                <li class="active"><a href="${pageContext.request.contextPath}/looksortServlet?nowpage=${i}&count=5&sortname=${sessionScope.sortname}&userid=${sessionScope.userid}">${i}</a></li>
                            </c:if>
                            <c:if test="${page.nowpage!=i}">
                                <li><a href="${pageContext.request.contextPath}/looksortServlet?nowpage=${i}&count=5&sortname=${sessionScope.sortname}&userid=${sessionScope.userid}">${i}</a></li>
                            </c:if>
                        </c:forEach>
                    </c:if>
                    <!--前半部分有多余页-->
                    <c:if test="${page.nowpage-2>1}">
                        <c:if test="${1==page.nowpage}">
                            <li class="active"><a href="${pageContext.request.contextPath}/looksortServlet?nowpage=1&count=5&sortname=${sessionScope.sortname}&userid=${sessionScope.userid}">1</a></li>
                        </c:if>
                        <c:if test="${1!=page.nowpage}">
                            <li><a href="${pageContext.request.contextPath}/looksortServlet?nowpage=1&count=5&sortname=${sessionScope.sortname}&userid=${sessionScope.userid}">1</a></li>
                        </c:if>
                        <li><a>...</a></li>
                    ...
                    <c:if test="${page.nowpage+2<page.totalpage}">
                        ...
                        <c:if test="${page.totalpage==page.nowpage}">
                            <li class="active"><a href="${pageContext.request.contextPath}/looksortServlet?nowpage=${page.totalpage}&count=5&sortname=${sessionScope.sortname}&userid=${sessionScope.userid}">${page.totalpage}</a></li>
                        </c:if>
                        <c:if test="${page.totalpage!=page.nowpage}">
                            <li><a href="${pageContext.request.contextPath}/looksortServlet?nowpage=${page.totalpage}&count=5&sortname=${sessionScope.sortname}&userid=${sessionScope.userid}">${page.totalpage}</a></li>
                        </c:if>
                    </c:if>
                    <c:if test="${page.nowpage+2>=page.totalpage}">
                    ...
                    <li>
                        ...
                        <a href="${pageContext.request.contextPath}/looksortServlet?nowpage=${page.nowpage + 1}&count=5&sortname=${sessionScope.sortname}&userid=${sessionScope.userid}" aria-label="Next">
