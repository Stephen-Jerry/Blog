...
<div class="container-fluid">
    <div class="container-fluid shenzi">
        <ul class="nav nav-pills  nav-stacked">
            ...
            <hr>
            <li role="presentation" class="active"><a href="/pageServlet5?id=${sessionScope.id}">我的粉丝</a></li>
            <hr>
            ...
            <c:forEach items="${page.list}" var="user" varStatus="s">
                <div>
                            <img src="${user.touxiang}" class="img-circle touxiang3">
                             <a href="/otherServlet?bloguserid=${user.id}&userid=${sessionScope.id}"><h4 style="margin-left: 80px;margin-top: -50px">${user.username}</h4></a>
                </div>
                <hr>
            </c:forEach>
            <nav aria-label="Page navigation" style="text-align: center">
                <ul class="pagination">
                    ...
                    <li>
                        ...
                        <a href="${pageContext.request.contextPath}/pageServlet5?nowpage=${page.nowpage-1}&count=5&id=${id}" aria-label="Previous">
                    ...
                    <c:if test="${page.nowpage-2<=1}">
                        <c:forEach begin="1" end="${page.nowpage}" var="i">
                            <c:if test="${page.nowpage==i}">
                                <li class="active"><a href="${pageContext.request.contextPath}/pageServlet5?nowpage=${i}&count=5&id=${id}">${i}</a></li>
                            </c:if>
                            <c:if test="${page.nowpage!=i}">
                                <li><a href="${pageContext.request.contextPath}/pageServlet5?nowpage=${i}&count=5&id=${id}">${i}</a></li>
                            </c:if>
                        </c:forEach>
                    </c:if>
                    <!--前半部分有多余页-->
                    <c:if test="${page.nowpage-2>1}">
                        <c:if test="${1==page.nowpage}">
                            <li class="active"><a href="${pageContext.request.contextPath}/pageServlet5?nowpage=1&count=5&id=${id}">1</a></li>
                        </c:if>
                        <c:if test="${1!=page.nowpage}">
                            <li><a href="${pageContext.request.contextPath}/pageServlet5?nowpage=1&count=5&id=${id}">1</a></li>
                        </c:if>
                        <li><a>...</a></li>
                    ...
                    <c:if test="${page.nowpage+2<page.totalpage}">
                        ...
                        <c:if test="${page.totalpage==page.nowpage}">
                            <li class="active"><a href="${pageContext.request.contextPath}/pageServlet5?nowpage=${page.totalpage}&count=5&id=${id}">${page.totalpage}</a></li>
                        </c:if>
                        <c:if test="${page.totalpage!=page.nowpage}">
                            <li><a href="${pageContext.request.contextPath}/pageServlet5?nowpage=${page.totalpage}&count=5&id=${id}">${page.totalpage}</a></li>
                        </c:if>
                    </c:if>
                    <c:if test="${page.nowpage+2>=page.totalpage}">
                    ...
                    <li>
                        ...
                        <a href="${pageContext.request.contextPath}/pageServlet5?nowpage=${page.nowpage + 1}&count=5&id=${id}" aria-label="Next">
