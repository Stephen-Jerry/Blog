...
<head>
    ...
    <script>
        ...
        function  remove(id,noticeid) {
            if(document.getElementById("notice"+noticeid).innerHTML=="取消关注"){
                document.getElementById("notice"+noticeid).innerHTML="关注";
                $.post("/noticeServlet",{flag:0,userid:id,bloguserid:noticeid},function () {
                 })
            }else{
                document.getElementById("notice"+noticeid).innerHTML="取消关注";
                 $.post("/noticeServlet",{flag:1,userid:id,bloguserid:noticeid},function () {
                 })
            }
        }
    </script>
    ...
    <div class="container-fluid">
        ...
        <div class="collapse navbar-collapse" id="bs-example-navbar-collapse-1">
            ...
            <ul class="nav navbar-nav navbar-right">
                <a href="../blog/writeblog.jsp?per=${sessionScope.per}" target="_blank"><button type="button" class="btn btn-default navbar-btn" id="button">创作中心</button></a>
...
<div class="container-fluid">
    <div class="container-fluid shenzi">
        <ul class="nav nav-pills  nav-stacked">
            ...
            <hr>
            <li role="presentation" class="active"><a href="/pageServlet6?id=${sessionScope.id}">我的关注</a></li>
            <hr>
    ...
    <div class="container-fluid you">
        ...
        <c:if test="${page.totalpage!=0}">
            <c:forEach items="${page.list}" var="user" varStatus="s">
                <br>
                <div>
                    <img src="${user.touxiang}" class="img-circle touxiang3">
                    <a href="/otherServlet?bloguserid=${user.id}&userid=${sessionScope.id}"><h4 style="margin-left: 80px;margin-top: -40px">${user.username}</h4></a>
                    <button type="button" style="margin-left: 740px;margin-top: -35px;border-color: brown;" id="notice${user.id}" onclick="remove(${sessionScope.id},${user.id})" class="btn btn-default navbar-btn">取消关注</button>
                </div>
                <hr>
            </c:forEach>
            <nav aria-label="Page navigation" style="text-align: center">
                <ul class="pagination">
                    ...
                    <li>
                        ...
                        <a href="${pageContext.request.contextPath}/pageServlet6?nowpage=${page.nowpage-1}&count=5&id=${id}" aria-label="Previous">
                    ...
                    <c:if test="${page.nowpage-2<=1}">
                        <c:forEach begin="1" end="${page.nowpage}" var="i">
                            <c:if test="${page.nowpage==i}">
                                <li class="active"><a href="${pageContext.request.contextPath}/pageServlet6?nowpage=${i}&count=5&id=${id}">${i}</a></li>
                            </c:if>
                            <c:if test="${page.nowpage!=i}">
                                <li><a href="${pageContext.request.contextPath}/pageServlet6?nowpage=${i}&count=5&id=${id}">${i}</a></li>
                            </c:if>
                        </c:forEach>
                    </c:if>
                    <!--前半部分有多余页-->
                    <c:if test="${page.nowpage-2>1}">
                        <c:if test="${1==page.nowpage}">
                            <li class="active"><a href="${pageContext.request.contextPath}/pageServlet6?nowpage=1&count=5&id=${id}">1</a></li>
                        </c:if>
                        <c:if test="${1!=page.nowpage}">
                            <li><a href="${pageContext.request.contextPath}/pageServlet6?nowpage=1&count=5&id=${id}">1</a></li>
                        </c:if>
                        <li><a>...</a></li>
                    ...
                    <c:if test="${page.nowpage+2<page.totalpage}">
                        ...
                        <c:if test="${page.totalpage==page.nowpage}">
                            <li class="active"><a href="${pageContext.request.contextPath}/pageServlet6?nowpage=${page.totalpage}&count=5&id=${id}">${page.totalpage}</a></li>
                        </c:if>
                        <c:if test="${page.totalpage!=page.nowpage}">
                            <li><a href="${pageContext.request.contextPath}/pageServlet6?nowpage=${page.totalpage}&count=5&id=${id}">${page.totalpage}</a></li>
                        </c:if>
                    </c:if>
                    <c:if test="${page.nowpage+2>=page.totalpage}">
                    ...
                    <li>
                        ...
                        <a href="${pageContext.request.contextPath}/pageServlet6?nowpage=${page.nowpage + 1}&count=5&id=${id}" aria-label="Next">
