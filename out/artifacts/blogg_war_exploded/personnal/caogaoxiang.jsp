...
<head>
    ...
    <script>
        ...
        function remove(blogid){
            $.post("/removedraftServlet",{blogid:blogid},function () {
                $("#total").load("/pageServlet4?id=${sessionScope.id}");
            })
        }
    </script>
</head>
...
<div class="container-fluid">
    <div class="container-fluid shenzi">
        <ul class="nav nav-pills  nav-stacked">
            ...
            <hr>
            <li role="presentation" class="active"><a href="/pageServlet4?id=${sessionScope.id}">草稿箱</a></li>
            <hr>
    ...
    <div class="container-fluid you">
        ...
        <c:if test="${page.totalpage!=0}">
            <c:forEach items="${page.list}" var="user" varStatus="s">
                <div>
                    <h3 style="margin-top: 0px">${user.head}</h3>
                    <span >${user.time}</span>
                    <a href="/writedraftServlet?id=${user.id}&userid=${user.userid}&head=${user.head}" style="margin-left: 650px" target="_blank">[编辑]</a>
            ...
            <nav aria-label="Page navigation" style="text-align: center">
                <ul class="pagination">
                    ...
                    <li>
                        ...
                        <a href="${pageContext.request.contextPath}/pageServlet4?nowpage=${page.nowpage-1}&count=5&id=${id}" aria-label="Previous">
                    ...
                    <c:if test="${page.nowpage-2<=1}">
                        <c:forEach begin="1" end="${page.nowpage}" var="i">
                            <c:if test="${page.nowpage==i}">
                                <li class="active"><a href="${pageContext.request.contextPath}/pageServlet4?nowpage=${i}&count=5&id=${id}">${i}</a></li>
                            </c:if>
                            <c:if test="${page.nowpage!=i}">
                                <li><a href="${pageContext.request.contextPath}/pageServlet4?nowpage=${i}&count=5&id=${id}">${i}</a></li>
                            </c:if>
                        </c:forEach>
                    </c:if>
                    <!--前半部分有多余页-->
                    <c:if test="${page.nowpage-2>1}">
                        <c:if test="${1==page.nowpage}">
                            <li class="active"><a href="${pageContext.request.contextPath}/pageServlet4?nowpage=1&count=5&id=${id}">1</a></li>
                        </c:if>
                        <c:if test="${1!=page.nowpage}">
                            <li><a href="${pageContext.request.contextPath}/pageServlet4?nowpage=1&count=5&id=${id}">1</a></li>
                        </c:if>
                        <li><a>...</a></li>
                    ...
                    <c:if test="${page.nowpage+2<page.totalpage}">
                        ...
                        <c:if test="${page.totalpage==page.nowpage}">
                            <li class="active"><a href="${pageContext.request.contextPath}/pageServlet4?nowpage=${page.totalpage}&count=5&id=${id}">${page.totalpage}</a></li>
                        </c:if>
                        <c:if test="${page.totalpage!=page.nowpage}">
                            <li><a href="${pageContext.request.contextPath}/pageServlet4?nowpage=${page.totalpage}&count=5&id=${id}">${page.totalpage}</a></li>
                        </c:if>
                    </c:if>
                    <c:if test="${page.nowpage+2>=page.totalpage}">
                    ...
                    <li>
                        ...
                        <a href="${pageContext.request.contextPath}/pageServlet4?nowpage=${page.nowpage + 1}&count=5&id=${id}" aria-label="Next">
