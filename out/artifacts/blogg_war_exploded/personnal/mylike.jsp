...
<head>
    ...
    <script>
        ...
        function  remove(userid,bloguserid,blogid) {
            if(document.getElementById("notice"+blogid).innerHTML=="取消收藏"){
                document.getElementById("notice"+blogid).innerHTML="收藏";
                $.post("/cangServlet",{flag:0,userid:userid,bloguserid:bloguserid,blogid:blogid},function () {
                })
            }else{
                document.getElementById("notice"+blogid).innerHTML="取消收藏";
                $.post("/cangServlet",{flag:1,userid:userid,bloguserid:bloguserid,blogid:blogid},function () {
                })
            }
        }
    </script>
...
<div class="container-fluid">
    <div class="container-fluid shenzi">
        <ul class="nav nav-pills  nav-stacked">
            ...
            <hr>
            <li role="presentation" class="active"><a href="/pageServlet7?id=${sessionScope.id}">我的收藏</a></li>
            <hr>
        ...
        <c:if test="${page.totalpage!=0}">
            <c:forEach items="${page.list}" var="user" varStatus="s">
                <div>
                    ...
                    <button type="button" style="margin-left: 740px;margin-top: -35px;border-color: brown;" id="notice${user.id}" onclick="remove(${id},${user.userid},${user.id})" class="btn btn-default navbar-btn">取消收藏</button>
                </div>
                <hr>
            </c:forEach>
            <nav aria-label="Page navigation" style="text-align: center">
                <ul class="pagination">
                    ...
                    <li>
                        ...
                        <a href="${pageContext.request.contextPath}/pageServlet7?nowpage=${page.nowpage-1}&count=5&id=${id}" aria-label="Previous">
                    ...
                    <c:if test="${page.nowpage-2<=1}">
                        <c:forEach begin="1" end="${page.nowpage}" var="i">
                            <c:if test="${page.nowpage==i}">
                                <li class="active"><a href="${pageContext.request.contextPath}/pageServlet7?nowpage=${i}&count=5&id=${id}">${i}</a></li>
                            </c:if>
                            <c:if test="${page.nowpage!=i}">
                                <li><a href="${pageContext.request.contextPath}/pageServlet7?nowpage=${i}&count=5&id=${id}">${i}</a></li>
                            </c:if>
                        </c:forEach>
                    </c:if>
                    <!--前半部分有多余页-->
                    <c:if test="${page.nowpage-2>1}">
                        <c:if test="${1==page.nowpage}">
                            <li class="active"><a href="${pageContext.request.contextPath}/pageServlet7?nowpage=1&count=5&id=${id}">1</a></li>
                        </c:if>
                        <c:if test="${1!=page.nowpage}">
                            <li><a href="${pageContext.request.contextPath}/pageServlet7?nowpage=1&count=5&id=${id}">1</a></li>
                        </c:if>
                        <li><a>...</a></li>
                    ...
                    <c:if test="${page.nowpage+2<page.totalpage}">
                        ...
                        <c:if test="${page.totalpage==page.nowpage}">
                            <li class="active"><a href="${pageContext.request.contextPath}/pageServlet7?nowpage=${page.totalpage}&count=5&id=${id}">${page.totalpage}</a></li>
                        </c:if>
                        <c:if test="${page.totalpage!=page.nowpage}">
                            <li><a href="${pageContext.request.contextPath}/pageServlet7?nowpage=${page.totalpage}&count=5&id=${id}">${page.totalpage}</a></li>
                        </c:if>
                    </c:if>
                    <c:if test="${page.nowpage+2>=page.totalpage}">
                    ...
                    <li>
                        ...
                        <a href="${pageContext.request.contextPath}/pageServlet7?nowpage=${page.nowpage + 1}&count=5&id=${id}" aria-label="Next">
