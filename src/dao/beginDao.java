...
public class beginDao {
    public Page<Blog> begindao(int page, int count, String flag){
        try {
            if(page<=0){
                page=1;
            }
            Connection cn= JDBCUtils.getConnection();
            String sql="select count(*) total from blog where state!=? AND id in(select id from blogflag where flag=?)";
            ...
            String sql2="select * from blog where id in(select id from blogflag where flag=?) AND state!=? order by good DESC limit ?,?";
    ...
    public Page<Blog> begindao2(int page, int count, String flag){
        try {
            if(page<=0){
                page=1;
            }
            Connection cn= JDBCUtils.getConnection();
            String sql="select count(*) total from recommend";
            PreparedStatement ps=cn.prepareStatement(sql);
            ResultSet rs=ps.executeQuery();
            rs.next();
            Page<Blog> pg=new Page<Blog>();
            ...
            cn.close();
            Connection cn2=JDBCUtils.getConnection();
            String sql2="select * from blog where id in(select blogid from recommend) order by publicc DESC limit ?,?";
            PreparedStatement ps2=cn2.prepareStatement(sql2);
